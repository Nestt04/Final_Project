...
- Drag & drop user story vào sprint
...
```
http://localhost/pbl-tracker/public/
```
...
1. **Password Hashing**: `password_hash()` với BCRYPT
2. **SQL Injection Prevention**: PDO Prepared Statements
3. **XSS Prevention**: `htmlspecialchars()` cho output
4. **CSRF Protection**: Token validation
5. **Session Security**: Secure session configuration
6. **Input Validation**: Server-side validation
7. **Role-based Access Control**: Middleware kiểm tra quyền
8. **SQL Injection Testing**: Parameterized queries

---

## Performance Optimization

1. **Database Indexing**: Index trên foreign keys và search fields
2. **Query Optimization**: Sử dụng JOIN thay vì multiple queries
3. **Caching**: Session caching cho user data
4. **Lazy Loading**: Load data khi cần thiết
5. **AJAX**: Giảm full page reload
